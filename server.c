...
static int is_mostly_text(const unsigned char *buf, size_t n){
    ...
    return printable*100/n >= 85; // >=85% printable => likely text
}
...
int main(void){
    ...
    print_prompt(BLUE,"Server:", mybuf);
                ...
                if(ch==127 || ch==8){ // backspace
                    ...
                    if(strncmp(mybuf,"/send ",6)==0){
                        ...
                        if(!fp){
                            ...
                            mylen=0; mybuf[0]=0; print_prompt(BLUE,"Server:", mybuf);
                        }else{
                        ...
                        char tm[6]; now_hm(tm,sizeof tm);
                        printf(CLEAR_LINE GREEN "[%s] Client:%s %s\n" RESET, tm, RESET, mybuf); // local echo style
                        dprintf(cfd,"E:\n"); // already sent; ensuring finalize
                        mylen=0; mybuf[0]=0;
                        print_prompt(BLUE,"Server:", mybuf);
                    }
                } else {
            ...
            for(;;){
                ...
                *nl=0; // terminate line
                ...
                if(strncmp(line_start,"T:",2)==0){
                    ...
                    print_prompt(GREEN,"Client:", peerbuf);
                    ...
                    // FBEGIN:<name>:<bytes>
                    char *p=line_start+7;
                    char *colon=strchr(p,':'); if(!colon){/*bad*/} else {
